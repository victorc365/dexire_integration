...
def get_local_thumbnail(df_recipes:pd.DataFrame, recipe_id: str, thumbnail_column:str="thumbnail"):
    ...
    try:
        ...
        print(f"Error recovering the thumbnails {e}")
...
class DisplayRecipeState(State):
    ...
    async def run(self) -> None:
        ...
        try:
            ...
            text = selected_recipe['ingredients']
            if text.lower().startswith("ingredient:"):
                text = text[len("ingredient:"):].strip()
            elif text.lower().startswith("ingredients"):
                text = text[len("ingredients:"):].strip()
            msg = f"Ingredients: {text}"
            await self.send(prep_outgoing_message(self.agent.id,
                                            msg
                                            ))
            ...
            thumbnail = get_local_thumbnail(food_dataset, selected_id)
            print(f"Obtained thumbnail: {thumbnail}")
